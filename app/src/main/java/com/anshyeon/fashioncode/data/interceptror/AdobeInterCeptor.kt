...
class AdobeTokenInterceptor @Inject constructor(
    private val tokenRepository: TokenRepository
) : Interceptor, Authenticator {

    override fun intercept(chain: Interceptor.Chain): Response {
        return runBlocking {
            val accessToken = tokenRepository.getAdobeToken().first()
    ...
    override fun authenticate(route: Route?, response: Response): Request? {
        val token = runBlocking(Dispatchers.IO) {
            val refreshToken = tokenRepository.getAdobeRefreshToken()
            refreshToken?.let {
                tokenRepository.saveAdobeToken(it)
                it
            }
        } ?: return null
